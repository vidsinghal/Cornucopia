        ...
        if (AsmFiles.query.filter_by(architecture=architectureName, binary_name=filename, binary_hash=hashNumber).all() == []):
            ...
            if (len(Database) > 0):
                ...
                for items in Database:
                    ...
                    file_path_current = DOWNLOADS + "/" + str(architectureName) + "/" + str(filename) + "/" + str(binary_hash_seen) 
                    file_data_current = open(os.path.join(file_path_current), "rb").read()
                    current_file_text, arch_name = get_text_sec(file_data_current, filename)
                    file_compression_current = lzma.compress(current_file_text)               
                    data_concatenated = current_file_text +  text_section
                    combined_compression = lzma.compress(data_concatenated)            
                    ncd = (len(combined_compression) - min(len(file_compression_current), len(CurrentFileCompression))) / max(len(file_compression_current), len(CurrentFileCompression))                  
                    ncd_scores.append(ncd)
                
                if(len(ncd_scores) > 0):
                    print(ncd_scores)
                    return_value = sum(ncd_scores) / len(ncd_scores)
            ...
            flash('bin file successfully added')
            print('Added a new bin File to the database')
