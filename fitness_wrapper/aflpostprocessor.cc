...
extern "C" size_t afl_custom_post_process(Config *config, unsigned char *buf, size_t buf_size, unsigned char **out_buf){
    ...
    //check with clang if the .s file compiles down to a binary and doesn't give error
    //if it gives error then don't send to the server and compile the O0 using llc.
    //compiled file
    //std::string compiledFile(config->assembly_folder + config->pname);
    //compiledFile += ".s";
    std::string compiledBin(config->assembly_folder + config->pname);
    std::string clang_command("ulimit -f 10000000; " + config->llvm_directory + "clang -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
