...
extern "C" size_t afl_custom_post_process(Config *config, unsigned char *buf, size_t buf_size, unsigned char **out_buf){
    ...
    //std::string compiledFile(config->assembly_folder + config->pname);
    //compiledFile += ".s";
    std::string compiledBin(config->assembly_folder + config->pname);
    std::string clang_command;
    if ((config->arch).compare("mips") == 0){
        clang_command = std::string("ulimit -f 10000000; mips-linux-gnu-gcc -mfp32 -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
    }
    else if ((config->arch).compare("x86-64") == 0){
        clang_command = std::string("ulimit -f 10000000; " + config->llvm_directory + "clang -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
    }
    else if ((config->arch).compare("x86") == 0){
        clang_command = std::string("ulimit -f 10000000; " + config->llvm_directory + "clang -m32 -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
    }
    else if ((config->arch).compare("arm") == 0){
        clang_command = std::string("ulimit -f 10000000; arm-linux-gnueabi-gcc -mfloat-abi=softfp -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
    }
    ...
    if (WEXITSTATUS(compile_status_clang) != 0){
        ...
        //std::string compiledFile(config->assembly_folder + config->pname);
        //compiledFile += ".s";
        compiledBin = config->assembly_folder + config->pname;
        std::string clang_command;
        if ((config->arch).compare("mips") == 0){
            clang_command = std::string("ulimit -f 10000000; mips-linux-gnu-gcc -mfp32 -Wl,--unresolved-symbols=ignore-in-object-files " + compiledFile + " -o " + config->assembly_folder+config->pname);
        }
