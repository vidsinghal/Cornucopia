...
for func in funcs:
    ...
    cfunc = d.getDecompiledFunction()
    if cfunc != None:
        decomp_func_signature = cfunc.getSignature()
        print(decomp_func_signature)

        # TODO : this is a hack
        # get num args from signature
        print(decomp_func_signature)
        args_string = decomp_func_signature[decomp_func_signature.find("(")+1:decomp_func_signature.find(")")]
        if args_string == "void":
            decomp_num_args = 0
        else:
            decomp_num_args = len(args_string.split(",")) 
    else:
        decomp_num_args = 0
    ...
    while(blocks.hasNext()):
        ...
        bb_detail["edges"] = []
        bb_detail["size"] = bb.getNumAddresses()
        number_of_addresses += bb.getNumAddresses()
    ...
    func_detail["size"] = number_of_addresses
