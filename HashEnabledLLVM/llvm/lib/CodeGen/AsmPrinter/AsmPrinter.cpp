...
void AsmPrinter::emitFunctionBody() {
  ...
  for (auto &MBB : *MF) {
    ...
    for (auto &MI : MBB) {
        ...
        //want to see what type of instructions are printed. does this print target dependent X86 instructions??
        //outs() << MI; 
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  //Writing the architecture name to the assembly file
  //Custom code to output the architecture name to the assembly file, makes it easier since the server
  //Can just regex it and use it further
  const Triple &Target = TM.getTargetTriple();
  OutStreamer->AddComment( "The architecture name for the current machine is: " + Target.getArchName() );
